...
```bash
node /path/to/codex-agent-orchestrator/bin/cao.mjs skill install
node /path/to/codex-agent-orchestrator/bin/cao.mjs skill status
```
...
```bash
node /path/to/codex-agent-orchestrator/bin/cao.mjs skill install --skills-dir ~/.agents/skills
```
...
- **Codex App:** type `/CAO`, choose the **CAO** suggestion, then send the inserted skill mention. The current desktop app matches the display name without case sensitivity.
- **Codex CLI:** use `$cao`, or choose CAO through `/skills`.
- **Other clients:** use their native skill picker. An unselected bare `/CAO` is not a portable command across every Codex surface.

The installed skill is named `cao` and displayed as `CAO`. If an already-open conversation has a cached catalog, refresh its skills (the current desktop app provides **Force reload skills**), reopen the conversation, or restart the client if needed. Creating a new conversation is not inherently required. Official [skill guidance](https://learn.chatgpt.com/docs/build-skills) describes discovery, symlink support, `$` invocation, and refresh behavior; [CLI commands](https://learn.chatgpt.com/docs/developer-commands?surface=cli) document `/skills`.
...
This is an instruction-based workflow with a durable preference record, not a modification of Codex's application-wide defaults, a hook that enforces every model decision, or a background scheduler. If context is lost entirely, invoke CAO again and supply the original state directory and run ID when resuming active work.

## Inspect and change the preference directly

```bash
node bin/cao.mjs mode enable --thread THREAD_ID --project /path/to/project
node bin/cao.mjs mode enable --thread THREAD_ID --agent claude --max-parallel 4 --max-attempts 3
node bin/cao.mjs mode status --thread THREAD_ID
node bin/cao.mjs mode disable --thread THREAD_ID
```

`--profile ID` selects a saved execution preference; `--profile ""` clears it. `--state-dir PATH` selects an existing shared CAO state directory outside the target project. Mode commands only change preferences: they do not dispatch tasks, cancel workers, or alter a profile/provider. Resolve already-active work according to the user's request when disabling the default.
...
```bash
node /path/to/codex-agent-orchestrator/bin/cao.mjs skill uninstall
```

Pass the same `--skills-dir` used at installation. Uninstall removes only the link to this checkout, leaving the source, mode records, runs, and other skills intact. It does not turn off a preference already loaded in a conversation; ask Codex to stop using CAO there, or use `mode disable` for that identity.
